...
async function main() {
  ...
  const [, , email, adminFlag] = process.argv;
  if (!email || !adminFlag || !["true", "false"].includes(adminFlag)) {
    console.error(
      "Usage: node scripts/set-admin-claim.mjs <user-email> <true|false> (adds/removes from admin whitelist)",
    );
    process.exit(1);
  }
  ...
  console.log(
    `Updated admin whitelist for ${email}. allowed=${isAdmin}.`,
  );
}
